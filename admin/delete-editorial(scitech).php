...
if(isset($_GET['deledi'])){ 
	...
	header('Location: delete-editorial(scitech).php?action=deleted');
	exit;
} 
if(isset($_GET['sound'])){
	$ediID = $_GET['sound'];
	...
	$stmt->execute();
	$row = $stmt->rowCount();
	header('Location: delete-editorial(scitech).php?action=soundModified');
	exit;
}
...
<head>
    ...
    <title>All Blogs | TechSci Room</title>
...
<script language="JavaScript" type="text/javascript">
  function deledi(id, title)
  {
	  ...
	  {
	  	window.location.href = 'delete-editorial(scitech).php?deledi=' + id;
	  }
  }
  function sound(id, title)
  {
	  ...
	  {
	  	window.location.href = 'delete-editorial(scitech).php?sound=' + id;
	  }
  }
  </script>
                ...
                <div class="row">
				     <div class="col-lg-12">
                        ...
                        <div class="card" style="border: none;">
                            <div align="center">
                                <h4>All Blogs</h4> <a href='add-editorial(scitech).php'> <button type="button" class="btn btn-dark"><i class="fa fa-sticky-note"></i>  Add New Blog</button></a>
                            </div>
                            <div class="card-body">
                                    ...
                                    <?php
										try {
											$pages = new Paginator('8','p');
                                                       ...
                                                       $stmt = $db->query('SELECT ediID FROM editorialscitech');

                                                       //pass number of records to
                                                       $pages->set_total($stmt->rowCount());

                                                       $stmt = $db->query('SELECT * FROM editorialscitech ORDER BY ediID DESC '.$pages->get_limit());
                                          ...
                                          ?>
                                                <td>
                                                	...
                                                	<button type="button" class="btn btn-primary"><i class="fas fa-assistive-listening-systems	"></i> Sound</button></a>
                                                  </td>
                                                	<td> 
                                                	<?php 
                                                	if($row['status']){
